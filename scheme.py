...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        operands = operands.map(lambda arg: scheme_eval(arg, env))
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN Problem 21
        apply_without_evaluate = scheme_apply(self, operands, env)
        a = complete_eval(apply_without_evaluate)
        return scheme_eval(a, env)
        # END Problem 21
...
def do_and_form(expressions, env):
    ...
    while expressions is not nil:
        eval_exp = expressions.first
        expressions = expressions.second
        if expressions is nil:
            result = scheme_eval(eval_exp, env, True)
        else:
            result = scheme_eval(eval_exp, env)
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    bindings.map(lambda x: check_form(x, 2, 2))
    formals, vals = nil, nil
    while bindings is not nil:
        new_formal = bindings.first.first
        new_vals = scheme_eval(bindings.first.second.first, env)
        formals = Pair(new_formal, formals)
        vals = Pair(new_vals, vals)
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            if (isinstance(operator, MacroProcedure)):
                ...
                operands = rest.map(lambda arg: scheme_optimized_eval(arg, env))
